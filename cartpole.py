"""
Training script for CartPole-v1 environment.
"""
...
def train_actor_critic_optuna(trial, env_name, episodes=1000, max_steps=500, seed=0):
...
def train_actor_critic(env_name, episodes=1000, max_steps=500, gamma=0.99, lr_p=5e-4, lr_v=1e-3, seed=0,
    ...
    policy = PolicyNet(hidden_sizes=hidden_sizes).to(DEVICE)
    value = ValueNet(hidden_sizes=hidden_sizes).to(DEVICE)
    ...
    for ep in range(1, episodes + 1):
        ...
        if update_mode == "episode" and len(ep_states) > 0:
            ...
            # For transition i: next state is states[i+1] for i < len-1, or final state s for i == len-1
            with torch.no_grad():
                ...
                # Next values: for each transition, use value of next state
                next_values = all_values[1:]  # V(s1), V(s2), ..., V(s_T)
                
                # Mask out next values where done is True (no bootstrapping)
...
def optuna_search(env_name, n_trials=50, episodes=1000, max_steps=500, seed=0):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train Actor-Critic on CartPole-v1")
    ...
    parser.add_argument("--lr_p", type=float, default=0.001474, help="Policy learning rate")
    parser.add_argument("--lr_v", type=float, default=0.008751, help="Value learning rate")
    parser.add_argument("--gamma", type=float, default=0.9838, help="Discount factor")
    parser.add_argument("--entropy_coef", type=float, default=0.01, help="Entropy coefficient")
    parser.add_argument("--hidden_sizes", type=str, default="64,128", help="Hidden layer sizes (comma-separated)")
    ...
    parser.add_argument("--update_mode", type=str, default="episode", choices=["step", "episode"], help="Update mode: step or episode")
    
    args = parser.parse_args()
    
    # Parse hidden sizes
    hidden_sizes = [int(s.strip()) for s in args.hidden_sizes.split(",") if s.strip()]
