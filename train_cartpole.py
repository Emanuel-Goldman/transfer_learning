"""
Training script for CartPole-v1 with Optuna hyperparameter optimization.
"""
...
def train_with_pruning(trial, seed: int, max_episodes: int, eval_interval: int = 50):
    ...
    obs_dim = env_config['obs_dim']  # Actual env obs dim (4 for CartPole)
    action_dim = env_config['action_dim']
    action_type = env_config['action_type']
    solved_threshold = get_solved_threshold(env, ENV_NAME)
    
    # Verify observation padding consistency
    # Note: ActorCritic networks always use OBS_DIM (6) as input_dim,
    # regardless of obs_dim parameter. Observations are padded to OBS_DIM
    # in pad_observation() and select_action() methods.
    
    # Create agent
    agent = ActorCritic(
        obs_dim=obs_dim,  # Passed for reference, but network uses OBS_DIM internally
        action_dim=action_dim,
        action_type=action_type,
        hidden_sizes=hidden_sizes,  # e.g., [64, 64] for idx 0
        lr_policy=lr_policy,
        lr_value=lr_value,
        gamma=gamma,
...
def run_optuna_search(n_trials: int = 30, seed: int = 0, max_episodes: int = 500, eval_interval: int = 50):
    """Run Optuna hyperparameter search for CartPole-v1 with pruning."""
    print("=" * 60)
    print("Optuna Hyperparameter Search: CartPole-v1")
    print("=" * 60)
